...
const BRANCHING_NODE_TYPES = new Set([
	'choice', 'doTry', 'multicast', 'loadBalance'
]);
...
function createNode(id: string, type: NodeType, stepType: StepType, absolutePath: string, label?: string): Node {
	return {
		id,
		data: {
			type,
...
function generatePlaceholderNodesAndEdges(
	...
	nodes.push(createNode(placeholderId, ADD_NODE_TYPE, "add-step", absolutePath, label));
...
function processChoiceStep(
	...
	if (Array.isArray(choice?.when)) {
		choice.when.forEach((when: any, i: number) => {
			...
			const whenResult = processSteps(when.steps, nodes, edges, whenId, null, absolutePath);
			branchEndIds.push(whenResult.lastStepId);
		});

		// Always add placeholder for 'when' branches
		const placeholderResult = generatePlaceholderNodesAndEdges(nodes, edges, stepId, initialAbsolutePath, "Add when");
	...
	if (choice?.otherwise?.steps) {
		...
		nodes.push(createNode(otherwiseId, CAMEL_NODE_TYPE, "otherwise", absolutePath));
	...
	if (nextStepId && branchEndIds.length > 0) {
		...
		const placeholderId = generateUniqueId("add");
		nodes.push(createNode(placeholderId, ADD_NODE_TYPE, "add-step", initialAbsolutePath, "Add"));
		branchEndIds.forEach(endId => {
...
function processDoTryStep(
	...
	if (Array.isArray(doTry?.doCatch)) {
		doTry.doCatch.forEach((doCatch: any, i: number) => {
			...
			nodes.push(createNode(doCatchId, CAMEL_NODE_TYPE, "doCatch", absolutePath));
	...
	if (doTry?.doFinally) {
		...
		nodes.push(createNode(doFinallyId, CAMEL_NODE_TYPE, "doFinally", absolutePath));
	...
	const placeholderResult = generatePlaceholderNodesAndEdges(nodes, edges, stepId, initialAbsolutePath, "Add doCatch");
	branchEndIds.push(placeholderResult);
...
function processMulticastOrLoadBalanceStep(
	...
	const placeholderResult = generatePlaceholderNodesAndEdges(nodes, edges, stepId, absolutePath);
	branchEndIds.push(placeholderResult);
	...
	steps.forEach((branchStep: any, index: number) => {
		...
		if (branchStep[nodeType]?.steps) {
			...
			nodes.push(createNode(branchNodeId, CAMEL_NODE_TYPE, nodeType, branchPath));
	...
	if (nextStepId && branchEndIds.length > 0) {
		...
		// Add placeholder to connect all branches to the next step
		// need to remove last part of the absolutePath to ensure the placeholder give the correct path
		absolutePath = absolutePath.replace(/\.multicast.*$/, "").replace(/\.loadBalance.*$/, "");
		const placeholderId = generateUniqueId("add");
		nodes.push(createNode(placeholderId, ADD_NODE_TYPE, "add-step", absolutePath, "Add"));
		branchEndIds.forEach(endId => {
			edges.push(createEdge(generateUniqueId("edge"), endId, placeholderId));
		})
		branchEndIds.push(placeholderId);
...
function processSteps(
	...
	steps.forEach((step, i) => {
		...
		const stepId = currentNextStepId || generateUniqueId(`${nodeType}-${parentId}`);
		const absolutePath = `${initialAbsolutePath}.steps.${i}.${nodeType}`;

		// Determine next step ID
		currentNextStepId = i < steps.length - 1
		...
		if (BRANCHING_NODE_TYPES.has(nodeType)) {
			switch (nodeType) {
				case "choice":
					lastStepId = processChoiceStep(step, stepId, nodes, edges, currentNextStepId, absolutePath);
					break;
				case "doTry":
					lastStepId = processDoTryStep(step, stepId, nodes, edges, currentNextStepId, absolutePath);
					break;
				case "multicast":
				case "loadBalance":
...
export function jsonToTopologyBuilder(routes: Route[], routeId: string): ParsedTopologyModel {
	const nodes: Node[] = [];
	const edges: Edge[] = [];
	...
	if (!routeId) {
		generatePlaceholderNodesAndEdges(nodes, edges, "route.from", initialAbsolutePath, "Add route");
		return { nodes, edges };
	}
	...
	nodes.push(createNode(fromId, CAMEL_NODE_TYPE, route.route?.from.uri as StepType, 'route.from'));
	...
	if (routeSteps) {
		...
		if (lastNodeId && !lastNodeId.includes("add")) {
			generatePlaceholderNodesAndEdges(nodes, edges, lastNodeId, `${initialAbsolutePath}.steps`);
		}
	}
	return { nodes, edges };
}